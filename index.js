...
const commands = [
    ...
    new SlashCommandBuilder()
        .setName('painelpx')
        .setDescription('Envia o painel de compra do Hydra via Pix')
        .addStringOption(option => option.setName('valor').setDescription('Valor do produto'))
        .addStringOption(option => option.setName('descricao').setDescription('Descrição personalizada para o painel'))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        if (interaction.commandName === 'setup') {
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '🖼️ Thumbnail', value: config.panelThumb ? `[Link da Imagem](${config.panelThumb})` : 'Nenhuma', inline: true },
                    { name: '📝 Descrição Atual', value: `\`${config.panelDescription.substring(0, 150)}...\``, inline: false }
                );
        ...
        if (interaction.commandName === 'painel') {
            ...
            if (config.panelThumb && config.panelThumb.startsWith('http')) {
                embed.setThumbnail(config.panelThumb);
            }
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'conf_desc') {
            ...
            modal.addComponents(
                ...
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('p_thumb').setLabel('URL da Thumbnail (Imagem)').setStyle(TextInputStyle.Short).setValue(config.panelThumb).setRequired(false))
            );
            await interaction.showModal(modal);
        }
        ...
        if (interaction.customId === 'copy_pix') {
            await interaction.reply({ content: `${config.pixKey}`, ephemeral: true });
        }
    }
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId === 'modal_desc') {
            ...
            saveConfig();
            await interaction.reply({ content: '✅ Texto e Imagem do painel atualizados!', ephemeral: true });
        }
